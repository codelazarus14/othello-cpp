...
namespace OthelloSF {
	...
	void Button::setString(const std::string& str) {
		...
		// 1/2 (0.5) - 2/5 (0.4) = 0.1
		// also remove half of padding to align with bottom of inner (buttonRect) = / 20
		m_text.setOrigin(-buttonWidth * 0.1f, -buttonHeight * 0.1f + buttonHeight / 20);
	}
	...
	void Button::onPress(const std::function<void()>& callback) {
		...
		sf::Vector2f buttonPos = m_buttonRect.getPosition();
		sf::Vector2f textPos = m_text.getPosition();
		int offset = bounds.height / 20;
		m_buttonRect.setPosition(buttonPos.x + offset, buttonPos.y + offset);
		m_text.setPosition(textPos.x + offset, textPos.y + offset);
	}
